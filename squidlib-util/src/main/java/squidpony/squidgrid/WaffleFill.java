...
public class WaffleFill {
    ...
    protected Boolean observe()
    {
        ...
        for (int x = 0; x < FMX; x++) for (int y = 0; y < FMY; y++)
        {
            ...
            for (int t = 0; t < T; t++) if (wave[t].contains(x, y))
            {
                amount += 1;
                sum += stationary[t];
            }
        ...
        if (argminx == -1 && argminy == -1)
        {
            ...
            for (int x = 0; x < FMX; x++)
            {
                for (int y = 0; y < FMY; y++) for (int t = 0; t < T; t++) if (wave[t].contains(x, y))
                {
                    observed[x][y] = t;
                    break;
                }
        ...
        for (int t = 0; t < T; t++) distribution[t] = wave[t].contains(argminx, argminy) ? stationary[t] : 0;
        int r = DetailedMimic.weightedRandom(distribution, random.nextDouble());
        for (int t = 0; t < T; t++) wave[t].set(t == r, argminx, argminy);
    ...
    public WaffleFill(String encoded, int width, int height, boolean periodic, boolean black)
    {
        this.FMX = width;
        this.FMY = height;
        this.periodic = periodic;
        this.black = black;
        String[] strings = StringKit.split(encoded, "\uffff");
...
/*
        ...
        Func<Func<int, int, char>, char[]> tile = f =>
        {
            char[] result = new char[tilesize * tilesize];
            for (int y = 0; y < tilesize; y++) for (int x = 0; x < tilesize; x++) result[x + y * tilesize] = f(x, y);
            return result;
        };

        Func<char[], char[]> rotate = array => tile((x, y) => array[tilesize - 1 - y + x * tilesize]);
*/
        ...
        while (idx < max)
        {
            ...
            String tilename = tile.getName(); //xtile.Get<string>("name");
        ...
        for (int d = 0; d < 4; d++)
        {
            GreasedRegion tg = tempPropagator[d];
            for (int t1 = 0; t1 < T; t1++)
            {
                IntVLA sp = sparsePropagator[d][t1];
