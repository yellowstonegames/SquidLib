 ...
 * {@link #deserializeFromString(String)} to create the MarkovChar without needing any repeated analysis.
 * <br>
...
public class MarkovChar implements Serializable {
     ...
     * This method analyzes the pairings of letters in a (typically large) corpus text, including {@code "'"} as part
     * of words and some markers as their own "words." It only uses one preceding char to determine the subsequent char.
    ...
    /**
     * Returns a representation of this MarkovChar as a String; use {@link #deserializeFromString(String)} to get a
     * MarkovChar back from this String. The {@link #chars} and {@link #processed} fields must have been given values by
     * either direct assignment, calling {@link #analyze(CharSequence)}, or building this MarkovChar with the
    ...
    /**
     * Recreates an already-analyzed MarkovChar given a String produced by {@link #serializeToString()}.
     * @param data a String returned by {@link #serializeToString()}
     * @return a MarkovChar that is ready to generate words with {@link #chain(long)}
     */
    ...
    /**
     * Copies the String array {@link #chars} and the 2D jagged int array {@link #processed} into a new MarkovChar.
     * None of the arrays will be equivalent references, but the Strings (being immutable) will be the same objects in
     * both MarkovChar instances.
