 ...
 * containsKey()). You can also set the key and value at a position with {@link #putAt(int, double, int)}, or alter
 ...
 */
public class IntDoubleOrderedMap implements Serializable, Cloneable {
     ...
     */
    protected double defRetValue = Double.NaN;
     ...
     */
    public IntDoubleOrderedMap(final int[] keyArray, final double[] valueArray, final float f) {
     ...
     * @throws IllegalArgumentException if <code>k</code> and <code>v</code> have different lengths.
     */
    public IntDoubleOrderedMap(final int[] keyArray, final double[] valueArray) {
     ...
     */
    public void putAll(final int[] keyArray, final double[] valueArray)
    {
    ...
    private int insertAt(final int k, final double v, final int idx) {
        int pos;
        if (k == 0) {
            if (containsNullKey)
            {
    ...
    public double putAt(final int k, final double v, final int idx) {
     ...
     * @param increment a double to add to the value associated with {@code k}
    ...
    public final class MapEntry {
        ...
        public int hashCode() {
            return HashCommon.mix(key[index])
                    ^ HashCommon.mix(NumberTools.doubleToMixedIntBits(value[index]));
        }
        @Override
     ...
     * instead of entries.
     */
    public final class ValueIterator extends MapIterator implements ListIterator<Double>, Serializable {
        private static final long serialVersionUID = 0L;
    ...
    public final class ValueCollection extends AbstractCollection<Double> implements Serializable
    {
        private static final long serialVersionUID = 0L;
        ...
        public boolean contains(Object v) {
            return v instanceof Double && containsValue((Double)v);
        }
        public void clear() {
            IntDoubleOrderedMap.this.clear();
        }
    ...
    public IntDoubleOrderedMap clone() {
        ...
        try {
            c = (IntDoubleOrderedMap) super.clone();
            c.key = new int[n + 1];
            System.arraycopy(key, 0, c.key, 0, n + 1);
            c.value = new double[n + 1];
    ...
    public int hashCode() {
        ...
        for (int j = realSize(), i = 0, t = 0; j-- != 0;) {
            ...
            t = HashCommon.mix(key[i]) ^ HashCommon.mix(
                    NumberTools.doubleToMixedIntBits(value[i]) ^ HashCommon.INV_INT_PHI);
            h += t;
            i++;
        }
        // Zero / null keys have hash zero.
        if (containsNullKey)
            h += HashCommon.mix(NumberTools.doubleToMixedIntBits(value[n]) ^ HashCommon.INV_INT_PHI);
        return h;
    }
    ...
    public boolean equals(Object o) {
        if (o == this)
            return true;
        if (!(o instanceof IntDoubleOrderedMap))
            return false;
        IntDoubleOrderedMap m = (IntDoubleOrderedMap) o;
        if (m.size() != size())
            return false;
     ...
     */
    public double getAt(final int idx) {
        int pos;
     ...
     */
    public double removeAt(final int idx) {
     ...
     */
    public double randomValue(IRNG rng)
    {
     ...
     */
    public IntDoubleOrderedMap shuffle(IRNG rng)
    {
        if(size < 2)
            return this;
        order.shuffle(rng);
     ...
     */
    public IntDoubleOrderedMap reorder(int... ordering)
    {
     ...
     */
    public double alterCarefully(final int original, final int replacement) {
     ...
     * @return the value associated with original before, and replacement now
     */
    public double alter(final int original, final int replacement) {
        double v;
        int idx;
        if (original == 0) {
            if (containsNullKey) {
    ...
    public double[] getMany(int... keys)
    {
     ...
     */
    public double alterAtCarefully(int index, int replacement)
    {
