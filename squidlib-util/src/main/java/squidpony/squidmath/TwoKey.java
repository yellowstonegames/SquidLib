...
/**
 * An ordered bidirectional map-like data structure, with unique A items and unique B items updated together like a map
 ...
 * get collections that implement SortedSet of its A or B items.
 * Created by Tommy Ettinger on 10/25/2016.
 */
@Beta
public class TwoKey<A, B>
{
    protected Arrangement<A> keysA;
    protected Arrangement<B> keysB;

    /**
     * Constructs an empty TwoKey.
     */
    public TwoKey()
    {
    ...
    /**
     * Constructs a TwoKey with the expected number of indices to hold (the number of A and number of B items is always
    ...
    /**
     * Constructs a TwoKey from a pair of Iterables that will be processed in pairs, adding a unique A from aKeys if and
    ...
    public TwoKey(TwoKey<A, B> other)
    {
        if(other == null)
        {
            keysA = new Arrangement<>();
    ...
    public TwoKey(Arrangement<A> aItems, Arrangement<B> bItems)
    {
     ...
     * @return true if index is a valid index in the ordering of this TwoKey
     */
     ...
     * @param index an int index into this TwoKey
    ...
    /**
     * Gets a random A from this TwoKey using the given RNG.
    ...
    /**
     * Gets a random B from this TwoKey using the given RNG.
    ...
    /**
     * Changes an existing A key, {@code past}, to another A key, {@code future}, if past exists in this TwoKey
     * and future does not yet exist in this TwoKey. This will retain past's point in the ordering for future, so
     * the associated other key(s) will still be associated in the same way.
     * @param past an A key, that must exist in this TwoKey's A keys, and will be changed
     * @param future an A key, that cannot currently exist in this TwoKey's A keys, but will if this succeeds
    ...
    /**
     * Changes an existing B key, {@code past}, to another B key, {@code future}, if past exists in this TwoKey
     * and future does not yet exist in this TwoKey. This will retain past's point in the ordering for future, so
     * the associated other key(s) will still be associated in the same way.
     * @param past a B key, that must exist in this TwoKey's B keys, and will be changed
     * @param future a B key, that cannot currently exist in this TwoKey's B keys, but will if this succeeds
     * @return this for chaining
     */
     ...
     * yet exist in this TwoKey. The position in the ordering for future will be the same as index, and the same
    ...
    /**
     * Adds an A key and a B key at the same point in the ordering to this TwoKey. Neither parameter can be
    ...
    /**
     * Puts all unique A and B keys in {@code aKeys} and {@code bKeys} into this TwoKey. If an A in aKeys or a B
    ...
    /**
     * Adds an A key and a B key at the given index in the ordering to this TwoKey. Neither a nor b can be
     * present in this collection before this is called. If you want to change or update an existing key, use
    ...
    /**
     * Removes a given A key, if {@code removing} exists in this TwoKey's A keys, and also removes any keys
    ...
    /**
     * Removes a given B key, if {@code removing} exists in this TwoKey's B keys, and also removes any keys
     * associated with its point in the ordering.
    ...
    /**
     * Reorders this TwoKey using {@code ordering}, which have the same length as this TwoKey's {@link #size()}
     ...
     * @return a newly-created iterator over this TwoKey's A keys
     */
     ...
     * @return a newly-created iterator over this TwoKey's B keys
     */
