 ...
 * @test
 * @bug 4245809
 * @summary Basic test for UnorderedSet. (Based on OrderedSetTest, which is based on LinkedHashSetTest, which is based on SetBash)
 */
...
public class UnorderedSetTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i=0; i<numItr; i++) {
            Set s1 = new UnorderedSet();
            AddRandoms(s1, setSize);

            Set s2 = new UnorderedSet();
            ...
            if (new UnorderedSet(union).hashCode() != union.hashCode())
    ...
    static Set clone(Set s) throws Exception {
        ...
        clone = (method==0 ?  (Set) ((UnorderedSet)s).clone() :
                 (method==1 ? new UnorderedSet(Arrays.asList(s.toArray())) :
                  serClone(s)));
        if (!s.equals(clone))
