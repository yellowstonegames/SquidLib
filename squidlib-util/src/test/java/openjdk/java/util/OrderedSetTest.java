 ...
 * @test
 * @bug 4245809
 * @summary Basic test for OrderedSet.  (Based on LinkedHashSetTest, which is based on SetBash)
 */
...
public class OrderedSetTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i=0; i<numItr; i++) {
            ...
            if (new OrderedSet(union).hashCode() != union.hashCode())
    ...
    static Set clone(Set s) throws Exception {
        ...
        clone = (method==0 ?  (Set) ((OrderedSet)s).clone() :
                 (method==1 ? new OrderedSet(Arrays.asList(s.toArray())) :
                  serClone(s)));
        if (!s.equals(clone))
