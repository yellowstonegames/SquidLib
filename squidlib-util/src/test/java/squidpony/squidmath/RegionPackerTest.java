...
public class RegionPackerTest {
    ...
    public void printPacked(RoaringBitmap packed, int width, int height)
    {
        RegionPacker.printPacked(packed, width, height);
    }
    ...
    public int RBArrayMemoryUsage(RoaringBitmap[] arr)
    {
        int ctr = 0;
        for (int i = 0; i < arr.length; i++) {
            ctr += arr[i].getSizeInBytes();
        }
        return (((ctr + 12 - 1) / 8) + 1) * 8;
    }
    ...
    public void testTranslate() {
        RoaringBitmap packed = RoaringBitmap.bitmapOf(0, 1, 2, 3), squashed = RoaringBitmap.bitmapOf(0);
        RoaringBitmap translated = RegionPacker.translate(packed, -2, -2, 60, 60);

        assertTrue(squashed.equals(translated));
        ...
        false true
         */
        boolean[][] grid2 = new boolean[][]{new boolean[]{false, true}, new boolean[]{true, false}};
        boolean[][] grid3 = new boolean[][]{new boolean[]{false, false}, new boolean[]{true, true}};
        RoaringBitmap packed2 = RegionPacker.pack(grid2), packed3 = RegionPacker.pack(grid3);

        RoaringBitmap translated2 = RegionPacker.translate(packed2, 1, 0, 2, 2);
        assertTrue(packed3.equals(translated2));
    }

    @Test
    public void testUnion() {
        RoaringBitmap union = RegionPacker.unionPacked(RoaringBitmap.bitmapOf(
                300, 301, 302, 303, 304,
        ...
        System.out.println();
        */
        assertTrue(union.equals(RoaringBitmap.bitmapOf(
                290, 291, 292, 293, 294, 295, 296, 297, 298, 299, 300, 301, 302, 303, 304,
                311, 312, 313, 314, 315, 316, 317, 318,
                321, 322, 323, 324
        ))); //new short[]{290, 15, 6, 8, 2, 4},

        union = RegionPacker.unionPacked(RoaringBitmap.bitmapOf(
                300, 301, 302, 303, 304,
                311, 312, 313, 314, 315, 316, 317,318,
                321, 322, 323, 324),
                RoaringBitmap.bitmapOf(
        ...
        */
        ...
        RoaringBitmap intersect  = RegionPacker.intersectPacked(RoaringBitmap.bitmapOf(
                300, 301, 302, 303, 304,
                311, 312, 313, 314, 315, 316, 317,318,
                321, 322, 323, 324),
                RoaringBitmap.bitmapOf(
        ...
        System.out.println();
        */
        assertTrue(intersect.equals(RoaringBitmap.bitmapOf(300, 301, 311)));
        //assertArrayEquals(new short[]{300, 2, 9, 1}, intersect);

        intersect = RegionPacker.intersectPacked(RoaringBitmap.bitmapOf(
                300, 301, 302, 303, 304,
                311, 312, 313, 314, 315, 316, 317,318,
                321, 322, 323, 324),
                RoaringBitmap.bitmapOf(
        ...
        System.out.println();
        */
        assertTrue(intersect.equals(RoaringBitmap.bitmapOf(311)));
    ...
    public void testPackOptimalParameters()
    {
        ...
        short[] packed, floors = CoordPacker.pack(map, '.');
        RoaringBitmap packedRB;
        boolean[][] unpacked, unpackedRB;
        int ramPacked = 0, ramPackedRB = 0, ramBoolean = 0, ramDouble = 0;
        Coord viewer;
        for (int t = 0; t < 100; t++) {
            viewer = dungeonGenerator.utility.randomCell(floors);
            seen = fov.calculateFOV(resMap, viewer.x, viewer.y, FOV_RANGE, RADIUS);
            packed = CoordPacker.pack(seen);
            packedRB = RegionPacker.pack(seen);

            unpacked = CoordPacker.unpack(packed, seen.length, seen[0].length);
            unpackedRB = RegionPacker.unpack(packedRB, seen.length, seen[0].length);
            for (int i = 0; i < unpacked.length ; i++) {
                for (int j = 0; j < unpacked[i].length; j++) {
                    assertTrue((seen[i][j] > 0.0) == unpacked[i][j]);
                    assertTrue(unpackedRB[i][j] == unpacked[i][j]);
                }
            }
            ramPacked += arrayMemoryUsage(packed.length, 2);
            ramPackedRB += packedRB.getSizeInBytes();
        ...
        System.out.println("Average Memory used by packed RoaringBitmap (Appropriate):" +
                ramPackedRB / 100.0 + " bytes");
        ...
        System.out.println("Average Compression, RoaringBitmap vs. boolean[][] (Appropriate):" +
                100.0 * ramPackedRB / ramBoolean + "%");
        System.out.println("Average Compression, RoaringBitmap vs. double[][] (Appropriate):" +
                100.0 * ramPackedRB / ramDouble + "%");
        System.out.println("Average Compression, RoaringBitmap vs. short[] (Appropriate):" +
                100.0 * ramPackedRB / ramPacked + "%");
    ...
    public void testPackPoorParameters()
    {
        ...
        System.out.println("Average Memory used by packed short[] (Approaching Worst-Case):" +
                ramPacked / 100.0 + " bytes");
        System.out.println("Average Memory used by packed RoaringBitmap (Approaching Worst-Case):" +
                ramPackedRB / 100.0 + " bytes");
        ...
        System.out.println("Average Compression, short[] vs. double[][] (Approaching Worst-Case):" +
                100.0 * ramPacked / ramDouble + "%");
        System.out.println("Average Compression, RoaringBitmap vs. boolean[][] (Approaching Worst-Case):" +
                100.0 * ramPackedRB / ramBoolean + "%");
        System.out.println("Average Compression, RoaringBitmap vs. double[][] (Approaching Worst-Case):" +
                100.0 * ramPackedRB / ramDouble + "%");
        System.out.println("Average Compression, RoaringBitmap vs. short[] (Approaching Worst-Case):" +
                100.0 * ramPackedRB / ramPacked + "%");
        System.out.println("FOV Map stored for every cell, booleans, 30x70: " +
                arrayMemoryUsage2D(240, 240, arrayMemoryUsage2D(30, 70, 1)));
        System.out.println("FOV Map stored for every cell, floats, 30x70: " +
                arrayMemoryUsage2D(240, 240, arrayMemoryUsage2D(30, 70, 4)));
    }
    /*
    @Test
    public void testPackPoorParameters()
