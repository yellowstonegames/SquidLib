package squidpony.performance.alternate.mapping;
 ...
 * parts of this class are not necessary to generate. For example, you can enable river generation if you want by
 * setting {@link #generateRivers} to true, but it doesn't produce very good results right now.
 * <br>
...
public abstract class WMG implements Serializable {
    ...
    public boolean generateRivers = false;
...
//                    ay = (int)((qs * z + 1.0) * height * 0.5);
    ...
    public static class HyperellipticalFNMap extends WMG {
         ...
         * Always makes a 200x100 map.
         * Uses ClassicNoise as its noise generator, with 1.0 as the octave multiplier affecting detail.
         * If you were using {@link HyperellipticalFNMap#HyperellipticalFNMap(long, int, int, double)}, then this would be the
         * same as passing the parameters {@code 0x1337BABE1337D00DL, 200, 100, ClassicNoise.instance, 1.0}.
         */
        public HyperellipticalFNMap() {
            this(0x1337BABE1337D00DL, 200, 100, 1.0);
        }
         ...
         * @param mapHeight the height of the map(s) to generate; cannot be changed later
         */
        public HyperellipticalFNMap(int mapWidth, int mapHeight) {
            this(0x1337BABE1337D00DL, mapWidth, mapHeight, 1.0);
        }
         ...
         * @param mapHeight   the height of the map(s) to generate; cannot be changed later
         */
        public HyperellipticalFNMap(long initialSeed, int mapWidth, int mapHeight) {
            this(initialSeed, mapWidth, mapHeight, 1.0);
        }
         ...
         * @param octaveMultiplier used to adjust the level of detail, with 0.5 at the bare-minimum detail and 1.0 normal
         */
        public HyperellipticalFNMap(long initialSeed, int mapWidth, int mapHeight, double octaveMultiplier) {
            this(initialSeed, mapWidth, mapHeight, octaveMultiplier, 0.125, 2.5);
        }
        ...
        public HyperellipticalFNMap(long initialSeed, int mapWidth, int mapHeight,
            ...
            zPositions = new double[width][height];
            edges = new int[height << 1];
            terrain = new FastNoise(rng.nextInt(), terrainFreq * 0.8f, FastNoise.SIMPLEX_FRACTAL, (int) (0.5 + octaveMultiplier * 10));
...
//                qc = NumberTools.cos(lat);
                ...
                lon = (float) (asin(Z[(int)(0.5 + Math.abs(yPos*iry)*(Z.length-1))])*Math.signum(yPos));
                qs = NumberTools.sin(lon);
                qc = NumberTools.cos(lon);
                ...
                for (int x = 0/*, xt = 0*/; x < width; x++, xPos += i_uw) {
                    //th = lon * xPos;
                    th = (float)(xPos * irx / Math.abs(alpha + (1-alpha)*ProjectionTools.hyperellipse(yPos * iry, kappa)));
                    if(th < -3.141592653589793 || th > 3.141592653589793) {
                        //if(th < -2.0 || th > 2.0) {
                        heightCodeData[x][y] = 10000;
                        inSpace = true;
                    ...
                    yPositions[x][y] = ps;
                    zPositions[x][y] = qs;
                    heightData[x][y] = (h = terrainLayered.getSimplexFractal(pc +
            ...
            for (int y = 0; y < height; y++, yPos += i_uh) {
                ...
                for (int x = 0; x < width; x++) {
                    ...
                    heatData[x][y] = (h = (((heatData[x][y] - minHeat0) * heatDiff * hMod) + h + 0.6) * temp);
                    if (fresh) {
                        ps = Math.min(ps, (float) h); //minHeat0
                        pc = Math.max(pc, (float) h); //maxHeat0
                    }
                }
            }
            if(fresh)
            ...
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    ...
                    moistureData[x][y] = (temp = (moistureData[x][y] - minWet0) * wetDiff);
                    if (fresh) {
                        qs = Math.min(qs, (float) h);
                        qc = Math.max(qc, (float) h);
                        ps = Math.min(ps, (float) temp);
                        pc = Math.max(pc, (float) temp);
                    }
                }
            }
            if(fresh)
