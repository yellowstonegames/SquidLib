...
public class ThesaurusArchiver extends ApplicationAdapter {
	public static final String ARCHIVE_VERSION = "3-0-4";
	@Override
	public void create() {
		FileHandle squidFile = Gdx.files.local("archives/Thesaurus-" + ARCHIVE_VERSION + ".squid.txt");
		if(squidFile.exists()) {
			String archiveContents = squidFile.readString("UTF-8");
			String archiveContentsAlt = Gdx.files.local("archives/Thesaurus-" + ARCHIVE_VERSION + ".alt.txt").readString("UTF-8");
			...
			String archive = Thesaurus.archiveCategories();
			String archiveAlt = Thesaurus.archiveCategoriesAlternate();
			Gdx.files.local("archives/Thesaurus-" + ARCHIVE_VERSION + ".squid.txt").writeString(archive, false, "UTF-8");
			Gdx.files.local("archives/Thesaurus-" + ARCHIVE_VERSION + ".alt.txt").writeString(archiveAlt, false, "UTF-8");
