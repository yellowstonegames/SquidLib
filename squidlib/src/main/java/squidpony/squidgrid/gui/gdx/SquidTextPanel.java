 ...
 * @param <T>
 * 
 * @deprecated Use {@link TextPanel} instead
 * @see ButtonsPanel
 */
@Deprecated
public abstract class SquidTextPanel<T extends Color> extends GroupCombinedPanel<T> {
	 ...
	 * @param fg
	 */
	public SquidTextPanel(ISquidPanel<T> bg, ISquidPanel<T> fg) {
		super(bg, fg);
	}
	...
	public void putBorder() {
		...
		if (borderColor != null && 0 < borderSize) {
			...
			UIUtil.drawMarginsAround(null, x, y, w * bg.cellWidth(), h * bg.cellHeight(), borderSize,
					borderColor, borderStyle, zoomMultiplierX, zoomMultiplierY);
		}
	}
