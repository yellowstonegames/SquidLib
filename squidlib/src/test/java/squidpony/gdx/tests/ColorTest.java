        ...
        for (int i = 1; i < 256; i++) {
            ...
            reverse[i] =
                    (int) ((lumas[i] = lumaYOG(col)) * yLim)
                            | (int) (((cbs[i] = coYOG(col)) + 0.5f) * cbLim) << shift1
                            | (int) (((crs[i] = cgYOG(col)) + 0.5f) * crLim) << shift2;
...
//        System.out.println();
        ...
        for (int i = 1; i < 256; i++) {
            ...
            for (int yy = y + 2, rr = rev + 2; yy <= yLim; yy++, rr++) {
                ...
                crf = MathUtils.clamp(crf * adj + 0x1.8p-9f, -0.5f, 0.5f);
...
//                crf = (crf - 0.5f) * 0.96875f + 0.5f;
            ...
            for (int yy = y - 2, rr = rev - 2; yy > 0; rr--) {
                ...
                cbf = MathUtils.clamp(cbf * adj, -0.5f, 0.5f);
                crf = MathUtils.clamp(crf * adj - 0x1p-10f, -0.5f, 0.5f);
//                cbf = (cbf - 0.5f) * 0.984375f + 0.5f;
...
//                        | (int) ((crf + 0.5f) * 63) << 13;
            ...
            if (match >= 0) {
                for (int yy = y - 3, rr = rev - 3; yy > 0; yy--, rr--) {
                    ...
                    cbf = MathUtils.clamp(cbf * adj, -0.5f, 0.5f);
                    crf = MathUtils.clamp(crf * adj - 0x1p-11f, -0.5f, 0.5f);
