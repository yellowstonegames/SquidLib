 ...
 */
public class WorldMapViewPoliticalDemo extends ApplicationAdapter {
...
//    public int noiseCalls = 0, pixels = 0;  // debug
    ...
    private final float[] NATION_COLORS = new float[144];
...
//        noiseCalls = 0; // debug
        ...
        wmv.generate();
        political = fpm.generate(seed + 1000L, world, wmv.getBiomeMapper(), null, 50, 1.0);
        // earlier settings
...
//        pixels = 0;                              // for debugging how many pixels are drawn
        ...
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                if((bio[x][y] & 1023) < 42)
                    c = SColor.lerpFloatColors(cm[x][y], NATION_COLORS[political[x][y] & 127], nation);
                else
                    c = cm[x][y];
...
//            System.out.println((float) (noiseCalls) / pixels);
    ...
    public void render() {
        ...
        nation = NumberTools.swayTight((System.nanoTime() & 0xFFFFFFFFL) * 0x1p-31f);
        if(spinning) 
            rotate();
    ...
    public static void main(String[] arg) {
        ...
        new Lwjgl3Application(new WorldMapViewPoliticalDemo(), config);
    }
}
