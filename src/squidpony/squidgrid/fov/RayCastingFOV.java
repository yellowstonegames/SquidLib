...
public class RayCastingFOV implements FOVSolver {
    ...
    private float gap = 0.001f;//how much gap to leave from the edges when tracing rays
...
//        }
     ...
     */
    private boolean runLine(double x, double y, double angle, double currentLight) {
        //check that still in range
        double deltax = startx - x;
        double deltay = starty - y;
        double distance = Math.sqrt(deltax * deltax + deltay * deltay);
        if (distance > radius) {
            return true;//reached edge of vision
        }

        if (currentLight > 0 && map[(int) x][(int) y].getResistance(key) < 1f) {
            lightMap[(int) x][(int) y] = (float) Math.max(lightMap[(int) x][(int) y], currentLight);
            return runLine(x + step * Math.cos(angle), y + step * Math.sin(angle), angle, (force - decay * distance) * (1 - map[(int) x][(int) y].getResistance(key)));
        } else {
            return false;
        }
    }
    ...
    private float getNearLight(int x, int y) {
        ...
        float light = Math.max(Math.max(lightMap[x2][y],
                lightMap[x][y2]),
                lightMap[x2][y2]);
    ...
    private void lightObstacles() {
        ...
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                if (map[x][y].getResistance(key) > 0f) {//only light up non-transparent objects at the edge of view
