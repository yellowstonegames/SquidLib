 ...
 */
@Beta
public class MicroRippleFOV implements FOVSolver {
    ...
    public float[][] calculateFOV(float[][] map, int startx, int starty, float force, float decay, RadiusStrategy rStrat) {
        ...
        float[][] uberMap = new float[width][height];
        for (DirectionIntercardinal dir : DirectionIntercardinal.values()) {
            if (startx + dir.deltaX >= 0 && startx + dir.deltaX < width && starty + dir.deltaY >= 0 && starty + dir.deltaY < height
                    && map[startx + dir.deltaX][starty + dir.deltaY] < 1.0) {
                lightMap[startx + dir.deltaX][starty + dir.deltaY] = force;
                lightSurroundings(startx + dir.deltaX, starty + dir.deltaY);
                for(int x=0;x<width;x++){
                    for(int y=0;y<height;y++){
                        uberMap[x][y] = Math.max(uberMap[x][y], lightMap[x][y]);
                    }
                }
            }
        }
